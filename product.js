        ...
        const robj = {
            ...
            <button class="event-btn ${r.available>0 ? "available" : ""}" ${r.available===0 ? "disabled" : ""}>
                    ...
                    <span class="event-content">
                        <h5 class="event-name">${ID_PRODUCT_MAP[r.product_id].title} (${r.available}/${totalSlots} Available)</h5>
                        <p class="event-time">${date.toLocaleString('en-US', {hour: 'numeric', minute: 'numeric', hour12: true})}</p>
                    </span>
                </button>
            `
        }
